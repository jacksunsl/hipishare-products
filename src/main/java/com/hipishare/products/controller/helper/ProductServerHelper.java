...
public class ProductServerHelper {
	private static final Logger LOGGER = Logger.getLogger(ProductServerHelper.class);
	...
	protected RequestObject getRequestObject() {
		...
		if (null == data || "".equals(data)) {
			ProductSystemException.raise("2001");
		}
		if (null == sign || "".equals(sign)) {
			ProductSystemException.raise("2002");
		}
		if (null == timestamp || "".equals(timestamp)) {
			ProductSystemException.raise("2003");
		}
	...
	protected void validSign(RequestObject requestObject) throws Exception {
		...
		if (!sign.equalsIgnoreCase(requestObject.getSign())) {
			ProductSystemException.raise("2004");
		}
	}
